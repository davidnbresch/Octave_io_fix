...
## Created: 2014-10-31
## Updates:
## 2014-11-02 Add version check (only version 10 is supported)
...
function [chk, missing4] = __OXS_chk_sprt__ (jcp, dbug=0)
  ...
  if (jpchk >= numel (entries4))
    ...
    try
      ...
      ## Unload offending odfdom jar. Mind for case
      oxsentries = jcp (find (! cellfun ("isempty", (strfind (lower (jcp), "openxls")))));
      for ii=1:numel (oxsentries)
        ## Check if there's really an openxls.jar, or that "openxls" just happens
        ## to be somewhere in the full path name
        [~, fname, ext] = fileparts (oxsentries{ii});
        if (index (lower (fname), "openxls") && strcmpi (ext, ".jar"))
          ## We'll trust this is the offending jar
          if (dbug > 2)
            printf ("Removing unsupported openxls version from javaclasspath\n");
          endif
          javarmpath (oxsentries{ii});
        endif
      endfor
    end_try_catch
 endif
