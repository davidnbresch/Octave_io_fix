...
## Created: 2014-11-03
## Updates:
## 2014-11-09 Various fixes for memos, indexing into char arrays, etc
## 2014-11-11 Style fixes & comments added
## 2014-11-15 Provide empty variables in case recs = 0 (exploring)
## 2014-11-17 dBase IV
##
## References:
...
function [data, datinfo] = dbfread (fname, recs=[], cols=[], rd_erased='')
  ...
  fbyte = fread (fid, 1, "uint8");
  ## Provisional type check.
  ##   3 = dbase III+ w/o memos
  ##  83 = dbase III+ w memos
  ## 131 = dbase III w memos
  ## 139 = dbase IV w memos
  ## ... <to be cont'd>
  if (! (fbyte == 3 || fbyte == 83 || fbyte == 131 || fbyte == 139))
    error ("dbfread: unsupported file type, only dbase III[+], IV & V  supported.\n");
  endif
  if (fbyte == 83 || fbyte == 131 || fbyte == 139)
      ...
      if (getcol)
        ...
        switch (dbf(ii).fldtyp)
          ...
          case "L"
            ...
            data(2:end, regexpi (fld, "[yt]")) = true;
          case {"F", "N"}
            ## Numeric
            data(2:end, scol) = num2cell (str2double (fld));
          case "M"
